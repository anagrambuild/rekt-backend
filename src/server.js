...
async function executeHighLeverageTrade(req, res) {
  ...
  await retryWithBackoff(async () => {
    ...
    try {
      ...
      // Step 2: Auto-enable high leverage mode if needed and allowed
      if (!isHighLeverageEnabled && FEATURE_FLAGS.AUTO_ENABLE_HIGH_LEVERAGE) {
      ...
      // Step 3: If high leverage mode not enabled and auto-enable disabled, return error
      if (!isHighLeverageEnabled && !FEATURE_FLAGS.AUTO_ENABLE_HIGH_LEVERAGE) {
        return res.status(400).json({
          success: false,
          error: "High leverage mode required",
          message: `High leverage mode must be enabled for ${leverage}x trades`,
          requiresHighLeverageMode: true,
          suggestion:
            "Enable high leverage mode using the /api/user/enable-high-leverage endpoint first",
        });
      }
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        ...
        message:
          "walletAddress, tradeAmount, leverage, direction, and marketSymbol are required",
      });
    }
    ...
    if (!SUPPORTED_MARKETS.hasOwnProperty(marketSymbol)) {
      return res.status(400).json({
        success: false,
        error: "Unsupported market",
        message: `Market ${marketSymbol} is not supported. Supported markets: ${Object.keys(
          SUPPORTED_MARKETS
        ).join(", ")}`,
      });
    }
    ...
    // FEATURE FLAG: Choose between High Leverage Mode and Legacy Mode
    if (FEATURE_FLAGS.USE_HIGH_LEVERAGE_MODE) {
      if (FEATURE_FLAGS.DEBUG_TRADE_MODE) {
        console.log(
          `🚀 FEATURE FLAG: Using HIGH LEVERAGE MODE for ${leverage}x trade`
        );
      }
      ...
      try {
        ...
        console.error("❌ High leverage mode failed:", error.message);

        if (FEATURE_FLAGS.FALLBACK_TO_LEGACY) {
          console.log("🔄 FALLBACK: Switching to legacy mode...");
    ...
    // LEGACY MODE: Original implementation (default)
    if (FEATURE_FLAGS.DEBUG_TRADE_MODE) {
      console.log(`📊 FEATURE FLAG: Using LEGACY MODE for ${leverage}x trade`);
    }

    await retryWithBackoff(async () => {
      console.log(
        "🔗 Connecting to Solana mainnet via custom RPC with rate limiting..."
      );
      ...
      try {
        // Get market data - load accounts if needed
        // Get market index from supported markets
        const marketIndex = SUPPORTED_MARKETS[marketSymbol];
        console.log(`📊 Using market index ${marketIndex} for ${marketSymbol}`);

        console.log(`📊 Fetching ${marketSymbol} market data...`);
        ...
        if (!marketAcct) {
          ...
          if (!marketAcct) {
            throw new Error(
              `${marketSymbol} market not found after fetching accounts`
            );
          }
          console.log(`✅ ${marketSymbol} market found after loading accounts`);
        }

        // Fetch live oracle price
        console.log(`💰 Fetching live ${marketSymbol} oracle price...`);
        let currentPrice;
        try {
          ...
          currentPrice =
            oraclePriceData.price.toNumber() / PRICE_PRECISION.toNumber();
          console.log(`💰 Oracle price: ${currentPrice}`);
        } catch (oracleError) {
          console.warn(
            "⚠️ Failed to fetch oracle price, using market price as fallback:",
            oracleError.message
          );
          currentPrice =
            marketAcct.amm.oraclePrice.toNumber() / PRICE_PRECISION.toNumber();
        }
        ...
        console.log(
          "📊 Trade - User account keys:",
          Object.keys(activeUserAccount)
        );
        console.log(
          "📊 Trade - Current margin mode:",
          activeUserAccount.marginMode
        );
        console.log(
          "📊 Trade - Is margin trading enabled:",
          activeUserAccount.isMarginTradingEnabled
        );

        // Check if high leverage mode is needed for this trade
        const isHighLeverageTrade = leverage > 20;
        const isHighLeverageEnabled =
          activeUserAccount.marginMode &&
          activeUserAccount.marginMode.highLeverage !== undefined;

        console.log(
          `🚀 Trade requires high leverage mode: ${isHighLeverageTrade}`
        );
        console.log(`🚀 High leverage mode enabled: ${isHighLeverageEnabled}`);

        // If high leverage trade but mode not enabled, provide guidance
        if (isHighLeverageTrade && !isHighLeverageEnabled) {
          console.log(
            "⚠️ High leverage trade attempted without high leverage mode enabled"
          );
          return res.status(400).json({
            success: false,
            error: "High leverage mode required",
            message: `This ${leverage}x leverage trade requires high leverage mode to be enabled first.`,
            requiresHighLeverageMode: true,
            currentLeverage: leverage,
            currentMarginMode: activeUserAccount.marginMode,
            suggestion:
              "Enable high leverage mode using the /api/user/enable-high-leverage endpoint first, then retry this trade.",
          });
        }
        ...
        try {
          ...
          totalCollateral =
            driftClient.getUser().getTotalCollateral().toNumber() / 1e6;
          console.log(
            `📊 Trade - Method 1 - SDK getTotalCollateral(): $${totalCollateral}`
          );
        } catch (error) {
          ...
          try {
            ...
            totalCollateral = Math.max(freeCollateral, totalAccountValue);
            console.log(
              `📊 Trade - Method 2 - Free collateral: $${freeCollateral}, Total account value: $${totalAccountValue}`
            );
            ...
            if (activeUserAccount.spotPositions) {
              for (const spotPos of activeUserAccount.spotPositions) {
                if (spotPos.scaledBalance && !spotPos.scaledBalance.isZero()) {
                  ...
                  calculatedCollateral += balance;
                  console.log(
                    `📊 Trade - Method 3 - Spot position ${spotPos.marketIndex}: $${balance}`
                  );
                }
              }
            }
            ...
            totalCollateral =
              calculatedCollateral > 0 ? calculatedCollateral : 75.34; // Use actual amount from error
            console.log(
              `📊 Trade - Method 3 - Calculated total: $${totalCollateral}`
            );
          }
        }

        console.log(
          `📊 Trade - Final Drift account collateral: $${totalCollateral}`
        );

        // Also check user's wallet USDC balance since we can deposit as part of the trade
        console.log(
          "💰 Checking wallet USDC balance for additional collateral..."
        );
        let walletUsdcBalance = 0;
        try {
          const tokenAccounts = await connection.getTokenAccountsByOwner(
        ...
        try {
          console.log(
            "🔍 Trade - Attempting accurate Drift SDK margin calculation..."
          );
          ...
          console.log(
            `📊 Trade - Market loaded, oracle price: $${
              oraclePrice.price.toNumber() / 1e6
            }`
          );
          ...
          try {
            ...
            console.log(
              "📊 Trade - User account data verified through User class"
            );
          } catch (userError) {
            throw new Error(
              `Trade - User account verification failed: ${userError.message}`
            );
          }

          console.log(
            "📊 Trade - All account data verified, attempting SDK margin calculation..."
          );
          ...
          calculationMethod = "Drift SDK with Oracle Data";
          console.log(
            `✅ Trade - SDK calculated margin required: $${marginRequired}`
          );
        } catch (sdkError) {
          console.log(
            "⚠️ Trade - SDK margin calculation failed, using enhanced fallback:",
            sdkError.message
          );
          ...
          try {
            ...
            if (market && market.marginRatio) {
              // Use Drift's actual margin ratio for the selected market
              const marketMarginRatio = market.marginRatio.toNumber() / 1e4; // Convert from basis points
              const maxLeverage = Math.floor(1 / marketMarginRatio);

              console.log(
                `📊 Trade - ${marketSymbol} market margin ratio: ${(
                  marketMarginRatio * 100
                ).toFixed(2)}%`
              );
              console.log(
                `📊 Trade - ${marketSymbol} max theoretical leverage: ${maxLeverage}x`
              );
            ...
            console.log(
              "⚠️ Trade - Enhanced fallback failed:",
          ...
          if (marginRequired < minMarginRequired) {
            console.log(
              `⚠️ Trade - Fallback margin too low ($${marginRequired}), adjusting to minimum ($${minMarginRequired})`
            );
            marginRequired = minMarginRequired;
          }

          console.log(
            `💰 Trade - Fallback calculated margin required: $${marginRequired}`
          );
          console.log(
            `📊 Trade - Using ${leverage}x leverage (${(
              (marginRequired / positionValueUSD) *
              100
            ).toFixed(2)}% margin rate)`
          );
        ...
        console.log(
          `📊 Trade - Collateral check: Trade=$${tradeAmount}, Leverage=${leverage}x, Position=$${positionValueUSD}, Margin needed=$${marginRequired}, Available=$${totalAvailableCollateral}`
        );
        console.log(`📊 Trade - Calculation method: ${calculationMethod}`);

        if (totalAvailableCollateral < marginRequired) {
          console.log(
            `⚠️ Trade - Insufficient total collateral: Have $${totalAvailableCollateral}, need $${marginRequired}`
          );
          return res.status(400).json({
            success: false,
        ...
        console.log(
          `  - Available collateral: $${totalAvailableCollateral.toFixed(2)}`
        );
        console.log(`  - Can execute trade: ${canExecuteTrade}`);
        console.log(`  - ${marketSymbol} price: $${currentPrice.toFixed(2)}`);

        // Calculate asset quantity based on position size
        const assetAmount = positionValueUSD / currentPrice;
        const assetSymbol = marketSymbol.split("-")[0];
        console.log(`  - ${assetSymbol} quantity: ${assetAmount.toFixed(4)}`);

        // Use Drift SDK's convertToPerpPrecision method for proper baseAssetAmount
        const baseAssetAmount = driftClient.convertToPerpPrecision(assetAmount);
        ...
        console.log(
          `  - Base asset amount (SDK precision): ${baseAssetAmount.toString()}`
        );
        ...
        try {
          ...
          const detailedErr = new Error(
            `getPlacePerpOrderIx failed: ${driftError.message || driftError}`
          );
        ...
        if (tokenAccounts.value.length === 0) {
          throw new Error(
            "No USDC token account found. Please deposit USDC to your wallet first."
          );
        }

        const userTokenAccount = tokenAccounts.value[0].pubkey;

        // Use exact user input as deposit amount - no manipulation
        let depositAmount = parseFloat(tradeAmount);

        console.log(
          `💰 USER INPUT DEPOSIT: $${depositAmount} (exact user input, no manipulation)`
        );
        console.log(
          `🔍 TRADE DEBUG: leverage=${leverage}, userInput=$${tradeAmount}, positionValueUSD=$${positionValueUSD}`
        );

        console.log(
          `💰 ABOUT TO CREATE DEPOSIT INSTRUCTION WITH: $${depositAmount} USD = ${
            depositAmount * 1e6
          } lamports`
        );
        ...
        if (depositAmount > totalAvailableCollateral) {
          return res
            ...
            .json(
              createErrorResponse(
                new Error(`Insufficient collateral for trade`),
                `Trade requires more margin. Need $${depositAmount.toFixed(
                  2
                )}, have $${totalAvailableCollateral.toFixed(2)}`,
                400
              )
            );
        }
        ...
        // Get fresh wallet balance to avoid using stale data
        ...
        try {
          ...
          if (tokenAccounts.value.length > 0) {
            ...
            currentWalletBalance = parseFloat(
              accountInfo.value.uiAmount || "0"
            );
          }
        } catch (error) {
          console.warn(
            "⚠️ Failed to get fresh wallet balance, using cached value:",
            error.message
          );
          currentWalletBalance = walletUsdcBalance;
        }

        console.log(
          `💰 FRESH WALLET USDC BALANCE: $${currentWalletBalance.toFixed(
            2
          )} (was $${walletUsdcBalance.toFixed(2)})`
        );

        // Use exact user input - no manipulation or caps
        const finalDepositAmountUSD = depositAmount;

        console.log(
          `💰 DEPOSIT INSTRUCTION: Using exact user input $${finalDepositAmountUSD} USD`
        );
        ...
        res.json({
          ...
          transactionData: {
            ...
            feePayer: walletAddress,
            assetPrice: currentPrice,
            assetQuantity: assetAmount.toString(),
          },
          message: `${direction.toUpperCase()} order ready for ${marketSymbol} (${assetAmount.toFixed(
            4
          )} ${assetSymbol} @ $${currentPrice.toFixed(2)})`,
        });
      } finally {
        try {
          await driftClient.unsubscribe();
...
// Feature Flag Toggle endpoint - Switch between trade implementations
app.post("/api/admin/toggle-high-leverage-mode", async (req, res) => {
  try {
    const { enabled } = req.body;

    // Toggle the feature flag (in production, this would be stored in database/config)
    FEATURE_FLAGS.USE_HIGH_LEVERAGE_MODE = enabled === true;

    console.log(
      `🚀 ADMIN: High leverage mode ${enabled ? "ENABLED" : "DISABLED"}`
    );

    res.json({
      success: true,
      message: `High leverage mode ${enabled ? "enabled" : "disabled"}`,
      currentSettings: {
        USE_HIGH_LEVERAGE_MODE: FEATURE_FLAGS.USE_HIGH_LEVERAGE_MODE,
        AUTO_ENABLE_HIGH_LEVERAGE: FEATURE_FLAGS.AUTO_ENABLE_HIGH_LEVERAGE,
        FALLBACK_TO_LEGACY: FEATURE_FLAGS.FALLBACK_TO_LEGACY,
        DEBUG_TRADE_MODE: FEATURE_FLAGS.DEBUG_TRADE_MODE,
      },
    });
  } catch (error) {
